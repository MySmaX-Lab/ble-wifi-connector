        ...
        try:
            ...
            if state == BLEWiFiConnectorState.RESET:
                ...
                if error != BLEErrorCode.NO_ERROR:
                    MXLOG_DEBUG(f'Something getting wrong while BLE setup! error code: {error}', 'red')
                    await ble_advertiser.stop()
                    state = BLEWiFiConnectorState.RESET
                    continue
                ...
                if wifi_manager.check_connection():
                    MXLOG_DEBUG(f'WiFi connection success. SSID: {wifi_manager.get_connected_wifi_ssid()}', 'green')
                    state = BLEWiFiConnectorState.NETWORK_CONNECTED
                else:
                    if connect_try > 0:
                        MXLOG_DEBUG(f'Connect to SSID {wifi_manager.ssid} failed... (try: {connect_try})', 'yellow')
                        connect_try -= 1
                        state = BLEWiFiConnectorState.NETWORK_SETUP
                    else:
                ...
                if not wifi_manager.check_connection():
                    MXLOG_DEBUG(f'WiFi connection lost...', 'yellow')
            ...
            MXLOG_DEBUG("main_event_loop cancelled")
